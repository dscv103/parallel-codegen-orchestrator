...
class GitHubAutomationHandler:
        ...
        if results:
            ...
            for result in results:
                ...
                if result.status == TaskStatus.FAILED and result.error:
                    ...
                    error_msg = (
                        result.error[:MAX_ERROR_MESSAGE_LENGTH] + "..."
                        if len(result.error) > MAX_ERROR_MESSAGE_LENGTH
                        else result.error
                    )
                    details = f"`{error_msg}`"
    ...
    async def _post_results(
        ...
        # Handle single issue posting
        if "issue_number" in context:
            await self._post_to_single_issue(
                context["issue_number"],
                comment_text,
                summary,
            )

        # Handle single PR posting
        if "pr_number" in context:
            await self._post_to_single_pr(context["pr_number"], comment_text, summary)

        # Handle batch issue posting
        if "issue_numbers" in context:
            await self._post_to_multiple_issues(
                context["issue_numbers"],
                comment_text,
                summary,
            )

        # Handle batch PR posting
        if "pr_numbers" in context:
            await self._post_to_multiple_prs(
                context["pr_numbers"],
                comment_text,
                summary,
            )

    async def _post_to_single_issue(
        self,
        issue_number: int,
        comment_text: str,
        summary: dict[str, Any],
    ) -> None:
        """Post comment to a single issue."""
        try:
            logger.info(
            ...
            logger.info("comment_posted_to_issue_success", issue_number=issue_number)

        except GithubException as e:
            self._handle_github_error(e, f"issue #{issue_number}", summary, issue_number)
        except (OSError, ValueError) as e:
            self._handle_unexpected_error(e, f"issue #{issue_number}", summary, issue_number)

    async def _post_to_single_pr(
        self,
        pr_number: int,
        comment_text: str,
        summary: dict[str, Any],
    ) -> None:
        """Post comment to a single PR."""
        try:
            logger.info(
            ...
            summary["comments_posted"] += 1
            logger.info("comment_posted_to_pr_success", pr_number=pr_number)

        except GithubException as e:
            self._handle_github_error(e, f"PR #{pr_number}", summary, pr_number)
        except (OSError, ValueError) as e:
            self._handle_unexpected_error(e, f"PR #{pr_number}", summary, pr_number)

    async def _post_to_multiple_issues(
        self,
        issue_numbers: list[int],
        comment_text: str,
        summary: dict[str, Any],
    ) -> None:
        """Post comment to multiple issues."""
        for issue_number in issue_numbers:
            try:
                ...
                self._handle_batch_github_error(e, f"issue #{issue_number}", summary, issue_number)
            except (OSError, ValueError) as e:
                self._handle_batch_unexpected_error(
                    e,
    ...
    async def _post_to_multiple_prs(
        self,
        pr_numbers: list[int],
        comment_text: str,
        summary: dict[str, Any],
    ) -> None:
        """Post comment to multiple PRs."""
        for pr_number in pr_numbers:
            try:
                ...
                self._handle_batch_github_error(e, f"PR #{pr_number}", summary, pr_number)
            except (OSError, ValueError) as e:
                self._handle_batch_unexpected_error(e, f"PR #{pr_number}", summary, pr_number)
    ...
    def _handle_github_error(
        ...
        error_msg = f"Failed to post comment to {target}: {e}"
        logger.exception(
    ...
    def _handle_unexpected_error(
        ...
        error_msg = f"Unexpected error posting to {target}: {e}"
        logger.exception(
    ...
    def _handle_batch_github_error(
        self,
        e: GithubException,
        target: str,
        summary: dict[str, Any],
        ...
        """Handle GitHub API errors in batch operations."""
        error_msg = f"Failed to post to {target}: {e}"
        logger.exception(
    ...
    def _handle_batch_unexpected_error(
        self,
        e: Exception,
        target: str,
        summary: dict[str, Any],
        ...
        """Handle unexpected errors in batch operations."""
        error_msg = f"Unexpected error posting to {target}: {e}"
        logger.exception(
        ...
        try:
            ...
            logger.info(
                ...
                removed_labels=[
                    label
                    for label in current_labels
                    if label.startswith(self.config.status_label_prefix)
                ],
            )
