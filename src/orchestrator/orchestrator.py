...
class TaskOrchestrator:
    ...
    async def orchestrate_with_early_termination(
        ...
        Example:
            ...
            ... )
        """
        def _raise_critical_task_error(task_id: str, error_msg: str) -> None:
            """Helper function to raise critical task error."""
            critical_error_msg = f"Critical task '{task_id}' failed: {error_msg}"
            raise OrchestrationError(critical_error_msg, task_id=task_id)
        ...
        try:
            while self.executor.dep_graph.is_active():
                ...
                for i, task_result in enumerate(completed):
                    ...
                    if is_critical and is_failed:
                        ...
                        _raise_critical_task_error(task_id, error_msg)
            ...
            error_msg = f"Critical failure during orchestration: {e}"
            raise OrchestrationError(error_msg) from e
        else:
            logger.info(
