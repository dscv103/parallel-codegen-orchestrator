...
BUSY_AGENTS_COUNT = 3
IDLE_AGENTS_COUNT = 2
CONVERSION_ORG_ID = "999"
CONVERSION_ORG_ID_INT = 999
...
class TestAgentPoolInitialization:
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_pool_initialization_minimum_size(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_pool_initialization_maximum_size(self, _mock_agent_class):
        """Test initializing pool with maximum size."""
        pool = AgentPool(
            org_id=CONVERSION_ORG_ID,
            token=TEST_TOKEN,
    ...
    def test_pool_initialization_invalid_size_too_small(self):
        ...
        with pytest.raises(ValueError, match="must be between") as exc_info:
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_all_agents_start_idle(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_agents_have_sequential_ids(self, _mock_agent_class):
...
class TestAgentAllocation:
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_get_idle_agent_when_available(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_get_idle_agent_returns_first_idle(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_get_idle_agent_when_none_available(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_get_idle_agent_skips_failed_agents(self, _mock_agent_class):
...
class TestAgentStatusTransitions:
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_mark_busy_from_idle(self, _mock_agent_class):
        """Test marking an idle agent as busy."""
        pool = AgentPool(org_id=TEST_ORG_ID, token=TEST_TOKEN, max_agents=TWO_AGENT_POOL)
        agent = pool.agents[0]
        task_id = "task-123"
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_mark_busy_from_busy_raises_error(self, _mock_agent_class):
        ...
        with pytest.raises(ValueError, match=r"Cannot mark agent .* as busy") as exc_info:
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_mark_busy_from_failed_raises_error(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_mark_idle_from_busy(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_mark_idle_from_failed(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_mark_failed_from_busy(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_mark_failed_from_idle(self, _mock_agent_class):
...
class TestAgentRecovery:
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_reset_agent_from_failed(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_reset_agent_from_idle_raises_error(self, _mock_agent_class):
        """Test that resetting an idle agent raises ValueError."""
        pool = AgentPool(org_id=TEST_ORG_ID, token=TEST_TOKEN, max_agents=TWO_AGENT_POOL)
        agent = pool.agents[0]

        with pytest.raises(ValueError, match="Cannot reset agent") as exc_info:
            pool.reset_agent(agent)
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_reset_agent_from_busy_raises_error(self, _mock_agent_class):
...
class TestPoolStatistics:
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_get_stats_all_idle(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_get_stats_mixed_statuses(self, _mock_agent_class):
        ...
        assert stats["busy"] == IDLE_AGENTS_COUNT
        assert stats["failed"] == 1

    @patch("src.agents.agent_pool.Agent")
    def test_get_stats_all_busy(self, _mock_agent_class):
        ...
        assert stats["idle"] == 0
        assert stats["busy"] == BUSY_AGENTS_COUNT
        assert stats["failed"] == 0

    @patch("src.agents.agent_pool.Agent")
    def test_get_total_agents(self, _mock_agent_class):
...
class TestConcurrentAllocation:
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_multiple_allocations(self, _mock_agent_class):
        ...
        assert len(set(agent_ids)) == BUSY_AGENTS_COUNT

        # Verify pool statistics
        stats = pool.get_stats()
        assert stats["busy"] == BUSY_AGENTS_COUNT
        assert stats["idle"] == IDLE_AGENTS_COUNT

    @patch("src.agents.agent_pool.Agent")
    def test_allocate_all_agents(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_release_and_reallocate(self, _mock_agent_class):
...
class TestEdgeCases:
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_mark_failed_with_error_message(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_mark_failed_without_error_message(self, _mock_agent_class):
    ...
    @patch("src.agents.agent_pool.Agent")
    def test_single_agent_pool(self, _mock_agent_class):
    ...
    def test_org_id_conversion_to_int(self, mock_agent_class):
        ...
        AgentPool(org_id=CONVERSION_ORG_ID, token=TEST_TOKEN, max_agents=SINGLE_AGENT_POOL)

        # Verify Agent was called with int org_id
        mock_agent_class.assert_called_with(token=TEST_TOKEN, org_id=CONVERSION_ORG_ID_INT)
